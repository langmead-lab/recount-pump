...
```{r broad_counts}
if(fastq_filter == '>=') {
    taxids <- q(paste('SELECT sra.taxon_id FROM sra'))
} else {
    taxids <- q(paste(
      'SELECT sra.taxon_id FROM sra, fastq',
      'WHERE sra.run_accession = fastq.run_accession',
      '  AND fastq.FASTQ_FILES', fastq_filter, '0'))
}
...
if(fastq_filter == '>=') {
    species_table <- function(taxid) {
      q(paste(
        'SELECT * FROM sra, study, submission, run',
        'WHERE sra.platform = "ILLUMINA"',
        '  AND sra.library_strategy = "RNA-Seq"',
        '  AND sra.library_source = "TRANSCRIPTOMIC"',
...
```{r csvize}
for(species in names(runs_by_species)) {
  write.csv(runs_by_species[[species]], file=paste0(species, '.csv'))
}
```
...
```{r mysql_import}
import_to_local_mysql <- F
local_mysql_prefix <- 'test_'
if(import_to_local_mysql) {
    cfg <- read.ini('~/.recount/local_db.ini')
    con <- dbConnect(MySQL(), host=cfg$client$host, port=as.integer(cfg$client$port),
                     dbname="recount", cfg$client$user, cfg$client$password)
    for(species in names(runs_by_species)) {
      dbWriteTable(con, value=runs_by_species[[species]],
                   name = paste(local_mysql_prefix, species, sep='_'))
    }
}
```
