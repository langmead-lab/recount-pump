...
prefix="https://genome-idx.s3.amazonaws.com/recount/recount-ref/$ref"
s3prefix="s3://genome-idx/recount/recount-ref/$ref"
...
echo "aws s3 cp $s3prefix/gtf.tar.gz - 2> gtf.run | tar -zxvf - 2>> gtf.run" > detar.jobs
echo "aws s3 cp $s3prefix/fasta.tar.gz - 2> fasta.run | tar -zxvf - 2>> fasta.run" >> detar.jobs
...
#echo "STAR --genomeLoad LoadAndExit --genomeDir `pwd`/star_idx" >> s3dn.sh
echo "popd" >> s3dn.sh
