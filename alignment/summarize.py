...
with open('accuracy.csv', 'wt') as ofh:
    ofh.write('samp,tool,expt,paired,ann,twopass,inton,alignment,softclip,baselev,measure,value\n')
    for dir, subdirs, files in os.walk('output'):
        for fn in files:
            if fn == 'perf_summary' or fn == 'perf_intron_recovery_summary' or \
               fn == 'perf_mapping_accuracy_summary' or fn == 'perf_mapping_accuracy_SC_summary':
                ...
                if expt.endswith('_female'):
                    expt = expt[:-7]
                intron = 'T' if fn == 'perf_intron_recovery_summary' else 'F'
                ...
                with open(os.path.join(dir, fn), 'rt') as fh:
                    for ln in fh:
                        ...
                        if len(toks) == 3:
                            rec1 = [samp, tool, expt, paired, ann, pass2, intron, alignment, softclip, 'T', toks[0], toks[1]]
                            rec2 = [samp, tool, expt, paired, ann, pass2, intron, alignment, softclip, 'F', toks[0], toks[2]]
                            ofh.write(','.join(rec1) + '\n')
                            ofh.write(','.join(rec2) + '\n')
                        else:
                            rec = [samp, tool, expt, paired, ann, pass2, intron, alignment, softclip, 'F'] + toks
                            ofh.write(','.join(rec) + '\n')
