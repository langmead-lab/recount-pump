...
fgrep -v "##" ${ORIG_UNIONED_GTF}.gff | perl -ne 'chomp; $f=$_; @f=split(/\t/,$f); $f[3]--; print "".join("\t",@f)."\n";'  | sort -k1,1 -k4,4n -k5,5n > ${ORIG_UNIONED_GTF}.gff.bed.sorted

#get gene info
fgrep '	gene	' ${ORIG_UNIONED_GTF} | cut -f 1,4,5,7,9 | perl -ne 'chomp; ($c,$s,$e,$o,$ginfo)=split(/\t/,$_); $s--; $ginfo =~/gene_id\s+"([^"]+)";/; $gid=$1; print "$c\t$s\t$e\t$gid\t0\t$o\t2\n";' > ${ORIG_UNIONED_GTF}.genes.bed
