...
if __name__ == '__main__':
    ...
    if '--test' in sys.argv:
        ...
        # This is ~29MB
        ensembl_fasta = 'ftp://ftp.ensembl.org/pub/release-90/fasta'
        species = 'caenorhabditis_elegans'
        fa_prefix = 'Caenorhabditis_elegans.WBcel235'
        elegans_url = '%s/%s/dna/%s.dna.toplevel.fa.gz' % (ensembl_fasta, species, fa_prefix)
        elegans_md5 = '151011266f8af1de2ecb11ff4a43a134'

        ncbi_genomes = 'ftp://ftp.ncbi.nlm.nih.gov/genomes'
        acc = 'GCF_000840245.1_ViralProj14204'
        lambda_fa = ncbi_genomes + '/all/GCF/000/840/245/%s/%s_genomic.fna.gz' % (acc, acc)
        lambda_fa_md5 = '7e74fba2c9e1107f228dbb12bada5c1c'
        lambda_annot = ncbi_genomes + '/all/GCF/000/840/245/%s/%s_genomic.gff.gz' % (acc, acc)
        lambda_annot_md5 = 'df93ad945b341e7f1b225742f9d8ee6b'

        phix_fa = 'http://www.cs.jhu.edu/~langmea/resources/phix.fa'
        phix_md5 = 'a1c4cc91480cd3e9e7197d35dbba7929'
        ...
        class TestReference(unittest.TestCase):
            ...
            def test_simple_source_insert(self):
                src = Source(retrieval_method="url", url_1=elegans_url, checksum_1=elegans_md5)
                self.assertEqual(0, len(list(self.session.query(Source))))
                self.session.add(src)
                self.session.commit()
                sources = list(self.session.query(Source))
                self.assertEqual(1, len(sources))
                self.assertEqual(elegans_url, sources[0].url_1)
                self.assertEqual(elegans_md5, sources[0].checksum_1)
            ...
            def test_simple_sourceset_insert(self):
                src1 = Source(retrieval_method="url", url_1=elegans_url, checksum_1=elegans_md5)
                src2 = Source(retrieval_method="url", url_1=elegans_url, checksum_1=elegans_md5)
                src3 = Source(retrieval_method="url", url_1=elegans_url, checksum_1=elegans_md5)
            ...
            def test_download_all(self):
                src1 = Source(retrieval_method="url", url_1=lambda_fa, checksum_1=lambda_fa_md5)
                src2 = Source(retrieval_method="url", url_1=phix_fa, checksum_1=phix_md5)
                self.session.add(src1)
                self.session.add(src2)
                ss = SourceSet(sources=[src1, src2])
                self.session.add(ss)
                self.session.commit()
                an1 = Source(retrieval_method="url", url_1=lambda_annot, checksum_1=lambda_annot_md5)
